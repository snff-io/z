...
class Rules {
    ...
    public maxMagnitude: int = 50

void    updateMagnitude(pairGroup: { [key: string]: Pair }) {
        pairGroup["current"].magnitude += 0.1;
    }

boolean    updateStability(pairGroup: { [key: string]: Pair }) {
        let stability: int = 0;
        let pressure: int = 0;
...
void    flipPair(grid: Grid, flip: boolean, pairGroup: { [key: string]: Pair }) {
        if (flip) {
            if (pairGroup["current"].stability === 1) {
